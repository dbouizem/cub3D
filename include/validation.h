...
int		dequeue(struct s_node **queue, int *x, int *y);
void	free_queue(struct s_node *queue);
void	init_directions(int *dx, int *dy);

/* flood_fill_bfs.c */
int		check_enclosure(struct s_app *app, char **lines, int start);

/* flood_fill_core.c */
int		find_player(struct s_app *app, char **lines, int start);
